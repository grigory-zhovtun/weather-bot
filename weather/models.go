...
// API - просто обертка над токеном, но она позволяет тебе избавиться от глобальных
// переменных. Старайся таким образом решать проблему передачи параметров в функцию.
type API string
...
// Для поиска координат по названию города давай использовать просто map,
// не нужно каждый раз читать и парсить этот файл
type city struct {
	Name string
	Lat  string
	Lon  string
